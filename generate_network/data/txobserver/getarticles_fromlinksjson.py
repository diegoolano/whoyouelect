...
def get_article_text(artsoup,posttype,errorfile):
	if posttype == "normal":
		content = artsoup.find('div',{'class':'article-con'})                   #THIS IS UNIQUE
...
def main_meth(searchfor,savepath):
	...
	tofiles = [ f for f in listdir(mypath) if isfile(join(mypath,f)) and str(f).find("error") > -1 ]

	for i,l in enumerate(links):
		errorfile = searchfor+"-to_"+str(i)+"error.html" 
		if errorfile in tofiles or i > -1: 
			errorfile = ""
			...
			l['text'] = text
			l['news-source'] = "TEXAS OBSERVER"      #THIS IS UNIQUE
			l['article-num'] = "to_"+str(i)             #THIS IS UNIQUE
			if text != "":
