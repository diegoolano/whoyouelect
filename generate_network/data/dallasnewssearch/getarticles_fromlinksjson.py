...
def get_article_text(artsoup,posttype,errorfile):
	if posttype == "normal":
		content = artsoup.find('div',{'class':'entry-content'})                   #THIS IS UNIQUE
		...
		try:
			content = artsoup.find('div',{'class':'story-body'})                   #THIS IS UNIQUE
			if content != None and content != '':
				ps = content.find_all('p')						 #THIS IS UNIQUE
				print "WORKED WORKED"
...
def main_meth(searchfor,savepath):
	#searchfor = "Garnet_Coleman"
	searchfor = searchfor.replace("+","_")
	mypath = savepath + "/"+searchfor
	...
	dmnfiles = [ f for f in listdir(mypath) if isfile(join(mypath,f)) and str(f).find("error") > -1 ]

	for i,l in enumerate(links):
		errorfile = searchfor+"-dmn_"+str(i)+"error.html" 
		if errorfile in dmnfiles or i > -1: 
			errorfile = ""
			...
			soup = get_page(u)
			posttype = "normal"
			if "blog.dallasnews.com/" in u:
				posttype = "blog"
			text = get_article_text(soup,posttype,errorfile)

			l['text'] = text
			l['news-source'] = "DALLAS MORNING NEWS"      #THIS IS UNIQUE
			l['article-num'] = "dmn_"+str(i)             #THIS IS UNIQUE
			if text != "":
